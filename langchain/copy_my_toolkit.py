...
def get_embedding(text):
    response = client.embeddings.create(
        input=text,
        model="text-embedding-3-large"  # Pastikan modelnya yang sesuai, mungkin "text-embedding-3-small" jika itu yang kamu gunakan
    )
    return response.data[0].embedding

columns = ['index', 'id', 'item_name', 'city', 'department', 'year', 'description', 'detailed_description', 'amount', 'timestamp', 'keywords']
...
class My_SQLDatabaseToolkit(BaseToolkit):
    ...
    def get_tools(self) -> List[BaseTool]:
        ...
        intermediary_dataframe_tool_description = (
            "Execute a SQL query and retrieve the data as a DataFrame in JSON format. "
